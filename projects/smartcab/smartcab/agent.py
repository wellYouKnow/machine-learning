...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        for action in  ['forward', 'left', 'right', None]:
             Qvalues.append(self.Q["{}-{}-{}-{}-{}-{}".format(str(state[0]), str(state[1]), str(state[2]), str(state[3]), str(state[4]), str(state[5]))][action])
        maxQ = max(Qvalues)
        return maxQ 
    ...
    def createQ(self, state):
        ...
        initialValue = 0.0
        if self.learning:
            if self.Q.has_key("{}-{}-{}-{}-{}-{}".format(str(state[0]), str(state[1]), str(state[2]), str(state[3]), str(state[4]), str(state[5]))) == False:
                for action in ['forward', 'left', 'right', None]:
                    actionDic[action] = initialValue
                self.Q["{}-{}-{}-{}-{}-{}".format(str(state[0]), str(state[1]), str(state[2]), str(state[3]), str(state[4]), str(state[5]))] = actionDic
    ...
    def choose_action(self, state):
        ...
        if self.learning == False:
            ...
            if self.epsilon > random.random():
                ...
                action = actionDic.keys()[actionDic.values().index(maxQ)]
        return action
